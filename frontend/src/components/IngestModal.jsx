...
export default function IngestModal({ isOpen, onClose, onIngest, onUpload }) {
  ...
  const handleIngestSelected = () => {
    ...
    onIngest(Array.from(selectedIds));
      ...
      <div className="relative flex h-full max-h-[600px] w-full max-w-2xl flex-col overflow-hidden rounded-xl border border-white/10 bg-cs2-bg-card shadow-2xl">
        ...
        <div className="flex items-center justify-between border-b border-white/10 px-5 py-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="flex items-center gap-1.5 rounded-lg border border-white/10 bg-white/5 px-3 py-1.5 text-[10px] font-bold text-zinc-400 transition-all hover:bg-white/10 hover:text-white"
            >
              <Upload className="h-3.5 w-3.5" />
              本地浏览并导入
            </button>
            <button onClick={onClose} className="rounded-md p-1.5 text-zinc-500 hover:bg-white/5 hover:text-zinc-300">
              <X className="h-4 w-4" />
            </button>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 border-b border-white/5 bg-black/20 px-5 py-3">
          ...
          <button
            ...
            className="flex items-center justify-center rounded-md border border-white/10 p-1.5 text-zinc-400 hover:bg-white/5 hover:text-white"
          >
        ...
        {items.length > 0 && (
          <div className="flex items-center gap-2 border-b border-white/5 bg-black/10 px-5 py-2 text-[10px]">
            ...
            <button onClick={handleClearSelection} className="text-zinc-400 hover:text-zinc-200">
              清空
            </button>
        ...
        <div className="flex-1 overflow-y-auto p-2">
          {loading ? (
            ...
            <div className="flex h-32 flex-col items-center justify-center gap-2 text-zinc-600">
              ...
              <p className="text-xs">{search ? '没有找到匹配的待入库 Demo' : '没有待入库的 Demo，点击侧栏「刷新」扫描新文件'}</p>
            </div>
          ) : (
              ...
              {items.map((it) => {
                ...
                return (
                  <div
                    ...
                    className={`flex cursor-pointer items-center justify-between rounded-md border p-2.5 transition-colors ${selectedIds.has(it.id) ? 'border-cs2-orange/40 bg-cs2-orange/5' : 'border-transparent hover:bg-white/5'}`}
                  >
        ...
        {/* Footer */}
        <div className="flex items-center justify-between border-t border-white/10 bg-cs2-bg-dark px-5 py-3">
