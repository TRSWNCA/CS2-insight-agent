        ...
        <div className="flex items-center gap-6 group-hover:hidden animate-in fade-in duration-200">
          <div className="flex flex-col items-end gap-0.5">
            <div className="flex items-center gap-1.5">
               <span className={`h-1.5 w-1.5 rounded-full ${demo.status === "done" ? "bg-emerald-500 shadow-[0_0_5px_#10b981]" : "bg-blue-500"} animate-pulse`} />
               <span className={`text-[10px] font-bold uppercase ${demo.status === "done" ? "text-emerald-400" : "text-blue-400"}`}>
                 {demo.status === "done" ? "Parsed" : "Loaded"}
               </span>
            </div>
      ...
      <div className="flex flex-col p-2 px-3" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between gap-2 overflow-hidden">
          <div className="flex flex-1 items-center gap-1.5 overflow-x-auto no-scrollbar pb-0.5">
            <span className={`flex shrink-0 items-center gap-1 rounded-md px-1.5 py-0.5 text-[9px] font-medium border ${demo.status === "done" ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/20" : "bg-blue-500/10 text-blue-400 border-blue-500/20"}`}>
              <CheckCircle2 className="h-2.5 w-2.5" />
              {demo.status === "done" ? `解析于 ${demo.parsed_at ? new Date(demo.parsed_at).toLocaleDateString('zh-CN', { month: '2-digit', day: '2-digit' }) : '未知'}` : "待解析"}
            </span>
            {killTags.map((tag, i) => (
